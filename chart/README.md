...
圖種拿到的 `mapping = { xIdx, yIdxs:[...], agg, radarMax }`:`xIdx` 是類別欄、`yIdxs` 是一或多個數值欄、`agg` 是重複類別的彙總方式(`sum`/`avg`/`count`)、`radarMax` 是雷達圖的共用上限(`null`=自動)。各圖種自己決定怎麼用(例:圓餅只用第一個 Y;散布把 X 與第一個 Y 當兩個數值座標;雷達把多個 Y 當各指標)。
...
- 散布圖的 X 欄最好是數值;雷達圖適合「少數幾列 × 多個數值欄」,各軸共用同一個上限(可在「雷達圖上限」欄自訂,空白=自動取最大值取整;只有一列時就不會畫成死的規則多邊形)。
