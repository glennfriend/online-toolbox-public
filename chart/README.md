...
- **同一份資料可切換任何圖種**:長條 / 橫條 / 折線 / 區域 / 圓餅 / 環圈 / 散布 / 雷達;滑鼠移到圖種上會提示「適合的情境」。
...
- **輸出尺寸**:可選 600×400(預設)/ 1280×720 / 1200×630;畫面自動縮放到看得見整張,匯出時為該精確尺寸。
- 匯出 **PNG**(點陣)或 **SVG**(向量,可無限放大)。
- **儲存庫**:把目前輸入的資料存進瀏覽器本機(IndexedDB),之後點一下即可回填、可刪除(同 scrapbook 的儲存,但不需要「暫存」)。
- **範例**:標題列右側的「Example」下拉,選 JSON / CSV / TSV / Markdown 任一,直接把示範資料載入輸入區(皆為真實且具權威來源的資料:台灣六都、台灣歷年人口與出生死亡、美國各州人口)。
...
圖種拿到的 `mapping = { xIdx, yIdxs:[...], agg, radarMax }`:`xIdx` 是類別欄、`yIdxs` 是一或多個數值欄、`agg` 是重複類別的彙總方式(`sum`/`avg`/`count`)、`radarMax` 是雷達圖的共用上限(一律是具體數字,手動可改)。各圖種自己決定怎麼用(例:圓餅只用第一個 Y;散布把 X 與第一個 Y 當兩個數值座標;雷達把多個 Y 當各指標)。

改資料時,Y 的勾選會**依欄名沿用**(欄位還在就保留),不會被重置。輸入區是「文字 / 表格」兩種模式,右上角切換:文字模式貼/改原始資料,表格模式看解析後的結果。
...
- 散布圖的 X 欄最好是數值;雷達圖適合「少數幾列 × 多個數值欄」,各軸共用同一個上限,在「雷達圖上限」欄手動設定(預設依資料種一個合理值,「依資料」鈕可一鍵套用目前資料最大值)——這樣同一份圖刻度固定、可比較,也不會因為只有一列就畫成死的規則多邊形。
