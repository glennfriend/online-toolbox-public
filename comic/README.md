...
- 一格**盡量同畫風**(全卡通真人 `girl`/`girl_2`/`old_man`,或 `penguin`+`dog`);但**性別/年齡正確優先於畫風一致**
  —— 若混一下畫風才能選到對的角色(例:爸爸用 `old_man`、兒子用 `penguin`),那就混,別為了畫風硬塞不對的角色。
- 卡通真人是**固定表情**(情緒靠對白帶);企鵝可換表情(`grumpy`/`sleepy`/`wow`…)→ 情緒反轉的
  梗優先用企鵝。
- **性別/年齡要對得上**:角色若有明確性別/年齡(爸爸、兒子、老人、小妹…),挑符合的角色演。
  目前真人只有 `girl`(女)、`girl_2`(女)、`old_man`(男)——**沒有男孩/年輕男性**。若某角色沒有合適的
  真人可演,**寧可用中性動物(`penguin`/`dog`)也不要拿性別/年齡明顯不符的真人**(例:別用 `girl` 演兒子)。
  缺哪種角色就跟使用者要圖來補。
