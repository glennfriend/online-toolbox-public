...
背景 = **完全不透明**的整格底圖(cover 裁切)。**不分畫風**,全放 `raw/assets/backgrounds/`,
執行期由 `js/backgrounds.js` 索引,故事用 `panel.bg = <id>` 引用。配件去背後疊在背景上。
新增 = 丟圖進 `raw/assets/backgrounds/` + 在 `js/backgrounds.js` 加一筆 + 更新本檔。

來源:いらすとや(irasutoya,免費;同一作品用超過 20 張需付費授權)。bedroom 兩張為插畫。
...
| bedroom | bedroom.webp | 暖色插畫臥室(白天) | 臥室 · 房間 · 室內 · 床 · 窗戶 · 暖色 · 插畫 · 白天 |
| bedroom-night | bedroom-night.webp | 粉紫 lofi 臥室(夜晚) | 臥室 · 房間 · 室內 · 床 · 窗戶 · 夜晚 · lofi · 粉紫 |
