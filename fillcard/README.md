...
| `circle-image`（下拉:圓圈嵌圖) | 深綠橫條清單:左右交替的**圓形圖章** + 花體小標語(Kaushan Script)+ 粗體大標題(Poppins)。**標題/內文/img** 皆為插槽,項目數不限。圓圈可嵌圖(`img`),圖會自動裁成圓形(`clipPath` + `slice`);空的話顯示相片佔位圖示。字少版面,字型/字級都調校過。 |
