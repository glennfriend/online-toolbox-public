...
> 台南景點已於 2026-08-01 依區域拆成「台南中西區 / 台南安平 / 台南近郊」三組。
> 美食也要照同樣的區域切法,不要做成單一「台南 - 美食」。
...
| 方法 | 結果 |
|---|---|
| Nominatim 查中文地址 | **自信地回傳錯誤結果** —— 台南店家配到淡水、龜山、松山 |
| Photon 查中文地址 | 更糟 —— 配到**喀拉蚩(巴基斯坦)、阿根廷** |
| Nominatim 查店名 | 同上,完全不可靠 |
| Overpass 查街道幾何(全市範圍) | 「保安路」「中山路」「開元路」台南**多個行政區都有**,全市平均後算出 15–55 公里的荒謬跨距,座標完全錯誤 |
| Overpass 查街道幾何(限市區 bbox) | 保安路、大勇街、中山路在市區範圍內 **0 段** —— OSM 對台南市區小街涵蓋不足 |
| 內政部 TGOS 地址定位 API | 需註冊金鑰 |
| 內政部門牌電子地圖 | 只是各縣市系統的入口,台南那台一樣停電 |

結論:**台灣中文街道地址,除了政府門牌開放資料,沒有其他可用的免費地理編碼來源。**
與其塞不可靠的座標進去(街道級誤差 250–600m,而且只有 2/3 的點查得到),先不收。
`geocode-street-osm.mjs` 保留下來當紀錄,但**不建議用於台南**。
